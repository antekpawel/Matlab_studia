...
enthalpy_sum=(integral(@(t) zad1_heat_gas(t,tab1_moduls_cychex),tem1_mix1,tem1_mix2)-R.*log(tem1_mix2)).*xma+(integral(@(t) zad1_heat_gas(t,tab1_moduls_eth),tem1_mix1,tem1_mix2)-R.*log(tem1_mix2)).*xmb+enthalpy_correction_pg;
...
disp('Penga-Robinsona: '+string(z_pr_m(3)));
%Van der Wals... please.
%Soave RKS
...
enthropy_without_correction=(integral(@(t) zad1_enthropy(t,tab1_moduls_cychex,M1_cychex),tem1_mix1,tem1_mix2)-enthropy_correction1).*xa+(integral(@(t) zad1_enthropy(t,tab1_moduls_eth,M1_eth),tem1_mix1,tem1_mix2)-enthropy_correction1).*xb;
...
disp('Produkcja entropii: '+string(((enthropy_sum+enthropy_correction_pg_mixrule).*m1+q1_str./273.15)));
